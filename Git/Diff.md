* `git diff` only show differences of unstaged files. For showing differences for staged files `git diff --staged` should be used and For seeing all of the changes `git diff HEAD`.
